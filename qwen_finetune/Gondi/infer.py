"""
Run inference + scoring for finetuned Qwen2.5-0.5B checkpoints, across any
number of directions in one pass.

Qwen is a decoder-only causal LM finetuned on Qwen's ChatML template (see
qwen_finetune.py) -- inference here uses the exact same prompt format
(system + user turn, tokenizer.apply_chat_template(..., add_generation_prompt=True))
so generation matches what the model was trained to continue from. Each
checkpoint is self-contained (saved via trainer.save_model() +
    ...
    [
      {
        ...
        "checkpoint_path": "./qwen-gondi-en2tgt-finetuned",
        "src_col": "English",
        "tgt_col": "Gondi",
        "src_name": "English",
        "tgt_name": "Gondi"
...
MAX_LENGTH      = 256   # prompt token budget -- same as training (qwen_finetune.py)
...
def build_messages(src_name, tgt_name, src_text):
    # Same shape as training's build_messages() (qwen_finetune.py), minus the
    # assistant turn -- add_generation_prompt=True renders the prompt exactly
    # as the model saw it during SFT, up to where its response begins.
    return [
            ...
            for i in range(0, len(src_texts), BATCH_SIZE):
                ...
                prompts = [
                    tokenizer.apply_chat_template(
                        build_messages(src_name, tgt_name, str(t)),
