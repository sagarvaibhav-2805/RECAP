...
def extend_tokenizer_if_needed(
    ...
    """
    Conditionally extend the mT5 tokenizer with a custom SentencePiece model.

    Three modes — set via cfg["src_tokenizer"] or cfg["tgt_tokenizer"]:

      "mt5_default"
          Never extend. Use mT5's built-in vocabulary as-is.
          Use for: English, Hindi, Bengali and any language mT5 covers well.

      "custom_spm"
          Always train a SentencePiece model on the given corpus and
          extend mT5's vocabulary with new subword pieces. The UNK
          ratio check is skipped — extension runs unconditionally.
          Use for: tribal/unseen languages (Bhili, Mundari, Gondi …)
          where mT5 has little or no coverage.
    ...
    mode = cfg.get(mode_key, "mt5_default")
    log = _safe_logger(logger)

    if mode == "mt5_default":
        log.info(
            f"[TOKENIZER] {side.upper()} tokenizer mode=mt5_default — "
            "using mT5 vocabulary as-is (no extension)"
        )
    ...
    if mode == "auto":
        ...
        if unk_ratio < threshold:
            log.info(
                ...
                "skipping extension (mT5 default sufficient)"
            )
            return tokenizer, model
        log.info(
        ...
        log.warning(
            ...
            "Valid values: 'mt5_default', 'custom_spm', 'auto'. "
            "Defaulting to mt5_default (no extension)."
        )
        return tokenizer, model
    ...
    # Register src/tgt tags as additional special tokens if required (safety net)
...
def load_model_and_tokenizer(
    ...
    try:
        ...
        config.tie_word_embeddings = True  # mT5 typically ties word embeddings
...
class TranslationDataset(Dataset):
    def __init__(
        ...
        self.use_task_prefix = use_task_prefix
    ...
    def __getitem__(self, idx):
        ...
        # Format input string with task prefix for mT5 text-to-text format
        if self.use_task_prefix:
            prefix = f"translate {self.src_lang} to {self.tgt_lang}: "
            src_text = prefix + src_text

        # T5 tokenization handles sequences seamlessly.
...
def build_datasets(
    ...
    use_task_prefix = cfg.get("use_task_prefix", False)
...
def compute_bleu_chrf(
    ...
    for batch in dataloader:
        ...
        with torch.autocast(
            ...
            # For mT5 we do NOT set forced_bos_token_id, we let the model use default decoder_start_token_id.
            generated = (
