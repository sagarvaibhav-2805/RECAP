...
def _apply_tokenizer_extension(
    ...
    for side in ("src", "tgt"):
        ...
        if mode == "mt5_default":
            log.info(
                f"[TOKENIZER] {side.upper()} mode=mt5_default — "
...
class InferDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> Dict:
        ...
        # Format input string with task prefix for mT5
...
class InferCollator:
    ...
    Passes raw src_text strings through as a plain list.
    """
...
def _build_generate_kwargs(cfg: Dict) -> Dict:
    """Assemble generation kwargs with clean defaults for mT5 models."""
    kwargs: Dict = {
...
def run_inference(
    ...
    gen_kwargs = _build_generate_kwargs(cfg)
