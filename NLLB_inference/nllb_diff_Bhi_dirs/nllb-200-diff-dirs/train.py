...
#                 marker="o",
#                 markersize=4,
#                 color=colors[idx] if len(cols) > 1 else colors[0],
#                 linewidth=1.5,
#             )
...
#     model_prefix = os.path.join(save_dir, f"spm_{lang_name}")
...
#     corpus_path = os.path.join(save_dir, f"{lang_name}_corpus.txt")
...
#     logger.info(f"[TOKENIZER] Training SentencePiece tokenizer for {lang_name}")
...
# def compute_unknown_ratio(tokenizer, texts: List[str], sample_size: int = 1000):
...
#         unk_tokens += sum(1 for t in toks if t == tokenizer.unk_token)
#     if total_tokens == 0:
#         return 0.0
...
#             tok_ext_dir, f"spm_{cfg['direction_name']}_{side}.model"
#         )
...
#     log.info(
#         f"[TOKENIZER] [{side.upper()}] Adding {len(new_regular_tokens)} new subword tokens"
#     )
...
#     log.info(f"[TOKENIZER] Resized embeddings {old_vocab_size} -> {len(tokenizer)}")
#     return tokenizer, model


# def setup_lora(model, cfg: Dict):
#     if not cfg.get("use_lora", False) and not cfg.get("use_qlora", False):
#         return model
...
# def load_model_and_tokenizer(cfg: Dict, logger):
#     rank = int(os.environ.get("RANK", "0"))
#     model_name = cfg["model_name"]
#     offline_dir = cfg.get("offline_model_dir", None)
#     use_qlora = cfg.get("use_qlora", False)
#     attn_impl = (
#         "flash_attention_2" if cfg.get("use_flash_attention", False) else "eager"
#     )
...
#     local_ok = offline_dir is not None and os.path.isdir(offline_dir)
#     log = _safe_logger(logger)

#     try:
#         if not local_ok or offline_dir is None or not os.path.isdir(offline_dir):
#             raise FileNotFoundError(
#                 f"Offline directory not found or unavailable: {offline_dir}"
#             )
...
# def _make_dataset(df: pd.DataFrame, tokenizer, cfg: Dict) -> TranslationDataset:
#     """Helper: wrap a dataframe in a TranslationDataset with config-driven params."""
...
#         max_target_length=cfg.get("max_target_length", 256),
#     )


# def build_datasets(cfg: Dict, tokenizer, train_df: pd.DataFrame, val_df: pd.DataFrame):
#     return _make_dataset(train_df, tokenizer, cfg), _make_dataset(val_df, tokenizer, cfg)
...
#     collator = DynamicCollator(tokenizer=tokenizer, model=model)
...
#     use_ddp: bool,
# ):
#     train_loader, train_sampler = _make_dataloader(
#         train_dataset, tokenizer, model, cfg, rank, world_size, use_ddp,
#         shuffle=True, drop_last=True, batch_size=cfg["batch_size"],
#     )
#     val_loader, val_sampler = _make_dataloader(
#         val_dataset, tokenizer, model, cfg, rank, world_size, use_ddp,
...
#     use_ddp: bool,
# ):
#     """Build a DataLoader for the held-out test set (no shuffle, no drop_last)."""
#     loader, _ = _make_dataloader(
#         test_dataset, tokenizer, model, cfg, rank, world_size, use_ddp,
#         shuffle=False, drop_last=False,
#         batch_size=cfg.get("eval_batch_size", cfg["batch_size"]),
#     )
#     return loader
...
# @torch.no_grad()
# def run_validation_loss(model, dataloader, device, world_size):
#     model.eval()
#     total_loss = 0.0
#     total_tokens = 0.0
...
#         total_loss += outputs.loss.item() * toks
#         total_tokens += toks

#     val_loss = reduce_sum_scalar(total_loss, device, world_size) / max(
#         1.0, reduce_sum_scalar(total_tokens, device, world_size)
#     )
#     return val_loss
...
#             forced_bos_token_id = raw_model.config.decoder_start_token_id
...
#         with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
#             raw_model = model.module if isinstance(model, DDP) else model
#             generated = raw_model.generate(**generate_kwargs)
...
#     predictions = gather_object(predictions, world_size)
#     references = gather_object(references, world_size)
...
#     Computes loss, BLEU, and CHRF++ and saves results to
#     <train_dir>/test_results.json (rank-0 only).
#     """
#     log = _safe_logger(logger)
#     is_main = rank == 0

#     log.info(f"[TEST] Running final test-set evaluation on {len(test_df)} samples.")

#     # Build dataset + loader using the same helper used for val
#     test_dataset = _make_dataset(test_df, tokenizer, cfg)

#     # We need a temporary collator; reuse model (unwrapped) for collator init
#     raw_model = model.module if isinstance(model, DDP) else model
#     test_loader = build_test_dataloader(
#         test_dataset, tokenizer, raw_model, cfg, rank, world_size, use_ddp
#     )

#     test_loss = run_validation_loss(
#         model=model, dataloader=test_loader, device=device, world_size=world_size
#     )
#     test_ppl = safe_exp(test_loss)

#     test_bleu, test_chrf = compute_bleu_chrf(
#         model=model,
#         tokenizer=tokenizer,
...
#             "test_chrf": round(test_chrf, 4),
#             "test_samples": len(test_df),
#         }

#         results_path = os.path.join(train_dir, "test_results.json")
#         with open(results_path, "w", encoding="utf-8") as f:
#             json.dump(results, f, indent=2)

#         msg = (
#             f"[TEST] Final Results | "
#             f"Loss {test_loss:.4f} | PPL {test_ppl:.4f} | "
#             f"BLEU {test_bleu:.4f} | CHRF++ {test_chrf:.4f} | "
...
#     return test_loss, test_bleu, test_chrf
...
#     """
#     Load train / val / (optional) test DataFrames.

#     Three modes:
#       A) train_csv == val_csv  →  single-file random split (val_split fraction
#          held out for val; test_csv is still honoured separately if provided).
#       B) train_csv != val_csv  →  use each file directly.

#     In both modes test_csv is always a separate optional field.
#     Debug row limits (max_train_rows_debug, max_val_rows_debug,
#     max_test_rows_debug) are applied after loading.

#     Returns (train_df, val_df, test_df | None).
#     """
#     if cfg["train_csv"] == cfg["val_csv"]:
#         # ── Mode A: single-file split ──────────────────────────────────────
...
#     else:
#         # ── Mode B: explicit separate files ───────────────────────────────
...
#     # ── Test split (always optional & always a separate file) ─────────────
#     test_csv = cfg.get("test_csv")
#     if test_csv:
#         test_df = pd.read_csv(test_csv).dropna()
#         if cfg.get("max_test_rows_debug") is not None:
#             test_df = test_df.iloc[: cfg["max_test_rows_debug"]]
#     else:
#         test_df = None
...
# def train_direction(rank, world_size, local_rank, use_ddp, cfg):
...
#     logger = (
#         init_logger(os.path.join(train_dir, "train.log"), f"train_{direction}")
#         if is_main
#         else None
#     )
#     log = _safe_logger(logger)
...
#         print(
#             f"[DATA] train={len(train_df)} | val={len(val_df)} | "
#             f"test={len(test_df) if test_df is not None else 'N/A (no test_csv)'}"
#         )
...
#         tokenizer=tokenizer, model=model, texts=src_texts, cfg=cfg,
#         logger=logger, side="src", is_main=is_main, use_ddp=use_ddp,
#         force_train=not cfg.get("resume_from_checkpoint", True),
#     )
#     tokenizer, model = extend_tokenizer_if_needed(
#         tokenizer=tokenizer, model=model, texts=tgt_texts, cfg=cfg,
#         logger=logger, side="tgt", is_main=is_main, use_ddp=use_ddp,
#         force_train=not cfg.get("resume_from_checkpoint", True),
#     )
...
#     train_dataset, val_dataset = build_datasets(cfg, tokenizer, train_df, val_df)
#     train_loader, val_loader, train_sampler, val_sampler = build_dataloaders(
#         train_dataset=train_dataset, val_dataset=val_dataset,
#         tokenizer=tokenizer, model=model,
#         cfg=cfg, rank=rank, world_size=world_size, use_ddp=use_ddp,
#     )
...
#         optimizer=optimizer,
#         num_warmup_steps=cfg.get("warmup_steps", 500),
#         num_training_steps=total_steps,
#     )

#     scaler = GradScaler("cuda", enabled=cfg.get("use_fp16", False))
...
#             [1 if resume_ckpt is not None else 0], device=device
#         )
...
#             checkpoint_dir=resume_ckpt, model=model, optimizer=optimizer,
#             scheduler=scheduler, scaler=scaler, device=device,
#         )
#         if is_main:
#             print(f"[RESUME] Restored from Epoch {start_epoch - 1}. Continuing from Epoch {start_epoch}.")
#             print(f"[RESUME] Best Val Loss: {best_val:.6f} | Global Step: {global_step}")
...
#                 )
#                 loss = outputs.loss / cfg.get("gradient_accumulation_steps", 1)
...
#             if (step + 1) % cfg.get("gradient_accumulation_steps", 1) == 0:
#                 if scaler.is_enabled():
...
#                     epoch=f"{epoch + step / max(1, len(train_loader)):.1f}",
#                 )

#         train_loss = reduce_sum_scalar(epoch_loss_sum, device, world_size) / max(
#             1.0, reduce_sum_scalar(epoch_tokens, device, world_size)
#         )
#         val_loss = run_validation_loss(model=model, dataloader=val_loader, device=device, world_size=world_size)

#         train_bleu, train_chrf = compute_bleu_chrf(
#             model=model, tokenizer=tokenizer, dataloader=train_loader,
#             device=device, cfg=cfg, world_size=world_size,
#             max_samples=cfg.get("max_train_bleu_samples", 256),
#         )
#         val_bleu, val_chrf = compute_bleu_chrf(
#             model=model, tokenizer=tokenizer, dataloader=val_loader,
#             device=device, cfg=cfg, world_size=world_size,
#         )
...
#         if is_main:
#             active_model_path = cfg.get("offline_model_dir") or cfg["model_name"]
...
#                 f"Train CHRF++ {train_chrf:.4f} | Val CHRF++ {val_chrf:.4f} | "
...
#                 log.warning(f"[PLOT] Could not generate plots: {str(pe)}")

#             if val_loss < best_val:
#                 best_val = val_loss
#                 wait = 0
#                 checkpoint_dir = os.path.join(
#                     base_output_dir, f"checkpoint-epoch-{epoch}-bleu-{val_bleu:.2f}"
#                 )
#                 prev_best_path = os.path.join(base_output_dir, "_best_checkpoint_path.txt")
#                 if os.path.isfile(prev_best_path):
#                     with open(prev_best_path) as _f:
#                         prev_best = _f.read().strip()
#                     if os.path.isdir(prev_best) and prev_best != checkpoint_dir:
#                         import shutil
#                         shutil.rmtree(prev_best, ignore_errors=True)
#                         log.info(f"[CHECKPOINT] Removed old best → {prev_best}")

#                 save_checkpoint(
#                     save_dir=checkpoint_dir, model=model, tokenizer=tokenizer,
...
#                 log.info(f"[CHECKPOINT] New best saved → {checkpoint_dir}")
#                 best_checkpoint_dir = checkpoint_dir
#             else:
#                 wait += 1

#             latest_checkpoint_dir = os.path.join(
#                 base_output_dir, f"checkpoint-latest-epoch-{epoch}"
#             )
#             prev_latest_path = os.path.join(base_output_dir, "_latest_checkpoint_path.txt")
#             if os.path.isfile(prev_latest_path):
#                 with open(prev_latest_path) as _f:
#                     prev_latest = _f.read().strip()
#                 if os.path.isdir(prev_latest) and prev_latest != best_checkpoint_dir:
#                     import shutil
#                     shutil.rmtree(prev_latest, ignore_errors=True)
#                     log.info(f"[CHECKPOINT] Removed old latest → {prev_latest}")

#             save_checkpoint(
#                 save_dir=latest_checkpoint_dir, model=model, tokenizer=tokenizer,
#                 optimizer=optimizer, scheduler=scheduler, scaler=scaler,
#                 epoch=epoch, global_step=global_step, best_val=best_val,
#                 wait=wait, cfg=cfg,
#             )
...
#             log.info(f"[CHECKPOINT] Latest saved → {latest_checkpoint_dir}")
...
#     # ── Final test-set evaluation (runs after training or early stop) ─────
#     if test_df is not None:
#         if use_ddp:
#             dist.barrier()
#         if is_main:
#             log.info("[TEST] Training complete — running held-out test evaluation.")
#         evaluate_test_set(
#             model=model, tokenizer=tokenizer, test_df=test_df,
#             cfg=cfg, device=device, world_size=world_size, rank=rank,
#             use_ddp=use_ddp, train_dir=train_dir, logger=logger,
#         )
#     else:
#         if is_main:
#             log.info("[TEST] No test_csv provided — skipping test evaluation.")


# def restore_checkpoint(checkpoint_dir, model, optimizer, scheduler, scaler, device):
...
#     state = torch.load(trainer_state_path, map_location=device, weights_only=False)
...
#         and item.startswith("checkpoint-epoch-")
#     ]
#     if not checkpoints:
#         return None
...
#             direction_name = direction_cfg["direction_name"]
#             if rank == 0:
#                 print("\n" + "=" * 20 + f" Training: {direction_name} " + "=" * 20)
...
#                 use_ddp=use_ddp, cfg=direction_cfg,
#             )
...
def ensure_lang_tag_registered(
    ...
    code like "bhi_Deva" that isn't one of them silently tokenizes to <unk>
    ...
    subword vocabulary. If a low-UNK-ratio language (e.g. Devanagari-script
    Bhili/Gondi under "auto" mode) never triggers subword extension, its
    language tag never gets registered either.

    When this happens for tgt_lang, forced_bos_token_id resolution falls back
    to decoder_start_token_id, which for NLLB equals eos_token_id — forcing
    every generated sequence to be empty and BLEU/CHRF++ to be exactly 0.0
    for the entire run, even while the underlying model trains normally.
...
def compute_bleu_chrf(
    ...
    for batch in dataloader:
        ...
        for start in range(0, bsz, gen_batch_size):
            ...
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                gen = raw_model.generate(**generate_kwargs)
...
def train_direction(rank, world_size, local_rank, use_ddp, cfg):
    ...
    # ── Register any non-native language tags (e.g. bhi_Deva) ──────────────
    ...
    # registers tags as a side effect of subword-vocab extension).
