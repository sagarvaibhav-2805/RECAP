"""
Run inference + scoring for finetuned NLLB checkpoints, across any number of
directions in one pass.

Each entry in infer_config.json is one inference job: a CSV to translate, the
finetuned checkpoint to load, and the NLLB source/target language codes.
Checkpoints are self-contained (saved via trainer.save_model() +
tokenizer.save_pretrained() during training -- this includes any tribal
language special token / vocab extension registered at training time), so
each job loads its own tokenizer+model directly from checkpoint_path. No
shared base model or re-registration logic is needed here.
    ...
    [
      {
        ...
        "checkpoint_path": "./nllb-gondi-en2tgt-finetuned",
        "src_col": "English",
        "tgt_col": "Gondi",
        "src_lang": "eng_Latn",
        "tgt_lang": "gon_Deva"
...
def run_job(job, device, lock=None):
    ...
    print(f"  {src_col} [{src_lang}]  ->  {tgt_col or '?'} [{tgt_lang}]")
    ...
    if n_done >= total_rows:
        ...
        tokenizer = AutoTokenizer.from_pretrained(checkpoint_path, src_lang=src_lang)
        ...
        if forced_bos_id == tokenizer.unk_token_id:
            raise ValueError(
                f"[{job_name}] tgt_lang '{tgt_lang}' has no registered token in this "
        ...
        with torch.no_grad():
            for i in range(0, len(src_texts), BATCH_SIZE):
                ...
                if has_ref:
                    buf_ref.extend(ref_texts[i:i + len(chunk)])

                if (i // BATCH_SIZE) % 20 == 0:
                    print(f"  decoded {n_done + i + len(chunk)}/{total_rows}")
