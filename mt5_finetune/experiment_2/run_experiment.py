"""
Experiment 2: mT5 sampling-diversity ablation on Bhili <-> Hindi.

Same source sentences (SEED=42 sample over datasets/bhilli/test.csv, same
sampling logic as experiment_1) but instead of varying beam width, this
generates 6 outputs per sentence from 6 different sampling configurations,
all with num_beams=2 fixed (do_sample=True + num_beams=2 is HF's "beam
...
temperature/top_p/num_beams, since these aren't self-describing the way
beam widths were in experiment_1.

Run from this directory (mt5_finetune/experiment_2/) -- checkpoint paths
...
MAX_LENGTH      = 128   # same as mt5_finetune/*/infer.py
SEED            = 42
N_SAMPLES       = 5000
    ...
    with torch.no_grad():
        for batch_start in range(n_done, total_rows, BATCH_SIZE):
            ...
            for sc in CONFIGS:
                ...
                batch_prompts = [prefix + s for s in batch_src]
                enc = tokenizer(
